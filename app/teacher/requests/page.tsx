...
export default function TeacherRequestsPage() {
  ...
  return (
    <>
      ...
      <Card>
        <CardHeader title={<>Requests <span className="ml-2 text-sm font-normal text-gray-400">{requests.length}</span></>} />
            ...
            {requests.map((r) => (
              <tr key={r.id} className="hover:bg-slate-50 transition-colors align-top">
                <td className="px-5 py-3.5">
                  <span className="font-semibold text-gray-800">{r.course?.code}</span>
                ...
                <td className="px-5 py-3.5 text-xs text-gray-500 max-w-[160px]">{r.adminNote ?? "—"}</td>
