...
type JsonData = {
  ...
  timeSlots: { sortOrder: number; label: string }[];
  ...
  specialBlocks?: { day: string; label: string; appliesTo?: string }[];
};

const APPLY = process.argv.includes("--apply");
const VERSION_NAME = "Spring 2026";
const VERSION_EFFECTIVE_DATE = new Date("2026-05-10T00:00:00Z");
const EXPECTED_ROWS = 136;

const data: JsonData = JSON.parse(readFileSync(join(__dirname, "routine-data-july.json"), "utf-8"));
...
async function main() {
  log(
    `\n=== Routine Management System — 5 July Spring 2026 routine import — ${
      APPLY ? "APPLY MODE (will write to the database)" : "DRY RUN (no changes will be made)"
    } ===`
  );
  ...
  // TimeSlot has no unique constraint in the schema — match by sortOrder,
  // which is also what session rows key off in Phase E.
  for (const ts of data.timeSlots) {
    const existing = await prisma.timeSlot.findFirst({ where: { sortOrder: ts.sortOrder } });
    if (existing) {
      refReport.timeSlots.updated++;
      if (APPLY) await prisma.timeSlot.update({ where: { id: existing.id }, data: { label: ts.label } });
    } else {
      refReport.timeSlots.created++;
      if (APPLY) await prisma.timeSlot.create({ data: { label: ts.label, sortOrder: ts.sortOrder } });
    }
  }
  ...
  for (const [initials, name] of Object.entries(data.teachers)) {
    ...
    if (existing) {
      ...
      // Never overwrite userId — only name is synced on an existing row.
  ...
  for (const room of data.rooms) {
    ...
    if (existing) {
      // Never overwrite capacity on an existing room.
      refReport.rooms.updated++;
    } else {
      refReport.rooms.created++;
      if (APPLY) await prisma.room.create({ data: { name: room.name, capacity: room.capacity ?? 40 } });
    }
  }
  ...
  if (unknownInitials.length > 0) {
    ...
    log(`⚠ ${unknownInitials.length} unknown teacher initials found, covering ${totalUnknownSessions} session(s):`);
  ...
  const [dbTimeSlots, dbTeachers, dbRooms, dbBatches, dbCourses] = await Promise.all([
    prisma.timeSlot.findMany({ select: { sortOrder: true } }),
  ...
  const jsonSlotOrders = new Set(data.timeSlots.map((t) => t.sortOrder));
  ...
  const extra = {
    timeSlots: dbTimeSlots.filter((t) => !jsonSlotOrders.has(t.sortOrder)).map((t) => t.sortOrder),
  ...
  log(
    existingVersion
      ? `Version "${VERSION_NAME}" already exists (id ${existingVersion.id}); will reuse it and (re)publish it exclusively. Its sessions are being replaced in Phase D/E.`
  ...
  // ---------------------------------------------------------------------
  // Phase D: clear existing routine sessions — Alarm → Reschedule →
  // Session, in that order, and nothing else. A JSON snapshot of what's
  // about to be deleted is written first, as an extra safety net beyond
  // the doc's own instruction to back up via the TiDB console.
  // ---------------------------------------------------------------------
  log("\n--- Phase D: clear existing routine sessions ---");
  ...
  if (APPLY) {
    const [alarms, reschedules, sessions] = await Promise.all([
    ...
    const backupPath = join(backupDir, `pre-step31-${stamp}.json`);
    writeFileSync(backupPath, JSON.stringify({ alarms, reschedules, sessions }, null, 2));
    log(`Backed up ${alarms.length} alarm(s), ${reschedules.length} reschedule(s), ${sessions.length} session(s) to ${backupPath}`);
    ...
    log(`Deleted — Alarm: ${delAlarms.count}, Reschedule: ${delReschedules.count}, Session: ${delSessions.count}`);
  } else {
    log("Would back up current Alarm/Reschedule/Session rows, then delete all of them (Alarm, then Reschedule, then Session). No other table is touched.");
  }

  // ---------------------------------------------------------------------
  // Phase E: insert the new routine, expanding multi-slot entries into
  ...
  if (APPLY) {
    ...
    for (const ts of data.timeSlots) timeSlotIdBySlot.set(ts.sortOrder, -1);
  }
  ...
  log(`Rows to insert: ${rowsToCreate.length} (expected ${EXPECTED_ROWS})`);
  if (skipped.length > 0) {
  ...
  if (APPLY) {
    for (const row of rowsToCreate) {
      const { _teacherInitials, _roomName, _slot, ...createData } = row;
  ...
  if (data.specialBlocks && data.specialBlocks.length > 0) {
    for (const block of data.specialBlocks) {
      log(`  ${block.day} (last slot): "${block.label}" (${block.appliesTo ?? "n/a"}) — not inserted; consider posting it as a Notice.`);
    }
  } else {
    log("  None.");
  }
